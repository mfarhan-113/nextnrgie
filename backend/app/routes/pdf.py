...
def generate_estimate_pdf(contract_id: int, db: Session = Depends(get_db)):
    ...
    p.setFont("Helvetica", 12)
    estimate_number = f"{contract.command_number}" if hasattr(contract, 'command_number') else ''
    p.drawString(left + 170, y, estimate_number)
    # Update footer doc number
    try:
        p._doc_number = estimate_number
    except Exception:
        pass
    y -= line_height
    ...
    table_y = left_col_y - 100  # Position below chantier
    ...
    p.setLineWidth(0.7)  # Thicker borders
    ...
    row_y = table_header_y - 20
    while row_y > y_position:
        p.line(header_x, row_y, header_x + total_width, row_y)
        row_y -= 20
    
    # Draw bottom line
    p.setLineWidth(0.7)  # Ensure thick bottom line
    ...
    tva_amount = sum(item.total_ht * (getattr(item, 'tva', 0.0) / 100) for item in facture_items) if 'facture_items' in locals() else 0.0
    ...
    # Column lines and horizontal grid lines
    p.setLineWidth(0.7)
    current_x = header_x
    for h in headers[:-1]:
