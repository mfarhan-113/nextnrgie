...
async def add_salary(salary: SalaryCreate, db: AsyncSession = Depends(get_db)):
...
async def get_salaries(db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Salary).order_by(Salary.created_at.desc()))
